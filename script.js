...
function displayTask(task) {
    ...
    taskItem.innerHTML = `
        <span class="task-text">
            <b>${task.name}</b> <br>(Responsable: ${task.responsible})<br>
...
document.getElementById('searchTask').addEventListener('keydown', function(event) {
    if (event.key === 'Enter') {
        searchTasks();
    }
});
...
document.getElementById('uno').addEventListener('submit', function(e) {
    e.preventDefault();
    showPopup();
});

function showPopup() {
    var popup = document.getElementById('confMessage');
    var closeBtn = document.querySelector('.message .close');
